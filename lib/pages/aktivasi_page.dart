...
class _AktivasiPageState extends State<AktivasiPage> {
  ...
  Future<void> _requestOtp() async {
    ...
    if (bpr == null) {
      _showMessage('Aktivasi Gagal', 'Silakan pilih BPR terlebih dahulu.');
      return;
    }

    setState(() => _loading = true);
    final result = await _authService.requestActivationOtp(
      bprId: bpr.bprId,
    ...
    if (!result.isSuccess) {
      _showMessage('Aktivasi Gagal', result.message);
      return;
    }
  ...
  Future<void> _verifyOtp() async {
    ...
    setState(() => _loading = true);
    final result = await _authService.verifyActivationOtp(
  ...
  Future<void> _submitPassword() async {
    setState(() => _loading = true);
    final result = await _authService.submitActivation(
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        title: const Row(
          children: [
            ...
            SizedBox(width: 8),
            Text('Aktivasi Berhasil', style: TextStyle(fontSize: 16)),
          ],
        ),
        content: Text(
          result.message.isEmpty
              ? 'Akun berhasil diaktivasi. Silakan login menggunakan password baru.'
              : result.message,
  ...
  Widget _buildStepHeader() {
    const labels = ['Data Akun', 'Verifikasi OTP', 'Buat Password'];
    return Row(
  ...
  Widget _buildPasswordStep() {
    return Column(
      children: [
        ...
        const SizedBox(height: 24),
        _primaryButton('Aktifkan Akun', _submitPassword),
      ],
    );
  }
